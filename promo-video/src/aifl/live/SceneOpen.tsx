...
export const SCENE_OPEN_DEFAULTS = {
  wordmark: 'HEATWARPED WIKI',
  wordmarkSize: 118,
  kicker: 'FAN HUB · STEAM DEMO · RATFIELD',
  kickerSize: 22,
  noteLine1: 'Free demo,',
  noteLine2: 'verified guides.',
  noteSize: 34,
  ink: '#e9edf6',
  amber: '#e85d3a',
  muted: '#8ea6bf',
...
// cards[1] = Demo guide card — centre x is exactly 960 (page centre).
const MAIN = 1;
const CARD = layout.projects.cards[MAIN];
const MCX = CARD.x + CARD.w / 2; // ≈ 960
const MCY = CARD.y + CARD.h / 2; // ≈ 2133
const RADIUS = 4;
...
const PATCH = '#10141f';
const SLOT_AMBER = '#e85d3a';
const BEAM_CORE = 'rgba(255,145,106,0.98)';
          ...
          >
            ...
            <div
              style={{
                ...
                filter: 'blur(6px)',
                opacity:
                  0.6 * Math.min(1, lift + Math.max(0, (frame - 114) / 16)),
                pointerEvents: 'none',
              }}
            />
            ...
            {frame >= 142 && frame <= 212
              ? (() => {
                  ...
                  const noteZ =
                    92 + Math.sin(((frame - 142) / 44) * Math.PI * 2) * 3;
                    ...
                    >
                      ...
                      <div
                        style={{
                          ...
                          background:
                            'radial-gradient(ellipse at 50% 50%, rgba(40,30,20,0.3), transparent 70%)',
                          filter: 'blur(12px)',
            ...
            >
              {WORDMARK.split('').map((ch, i) => {
                ...
                const glint = interpolate(
                  frame,
                  [glintCenter - 4, glintCenter, glintCenter + 4],
                  [0, 1, 0],
                  {
                    extrapolateLeft: 'clamp',
                    extrapolateRight: 'clamp',
